...
pub fn parse_oauth_callback(input: &AuthExchangeInput) -> Result<OAuthCallbackPayload> {
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        if payload.code.is_none() {
            payload.code = parsed
                .code
                .or_else(|| Some(raw_code.split('#').next().unwrap_or(raw_code).to_string()));
        }
        if payload.state.is_none() {
            payload.state = parsed.state;
        }
...
pub fn validate_callback_state(
    ...
    let Some(actual_state) = actual_state
        .map(str::trim)
        .filter(|value| !value.is_empty())
    else {
        return Ok(());
...
mod tests {
    ...
    fn parses_code_and_state_from_callback_url() {
        let payload = parse_oauth_callback(&AuthExchangeInput {
            callback_url: Some(
                "http://localhost:1455/auth/callback?code=auth-code-123&state=state-123"
                    .to_string(),
            ),
            ..Default::default()
        })
        ...
        assert_eq!(payload.code.as_deref(), Some("auth-code-123"));
        assert_eq!(payload.state.as_deref(), Some("state-123"));
    }

    #[test]
    fn parses_code_and_state_from_fragment_callback_url() {
        let payload = parse_oauth_callback(&AuthExchangeInput {
            callback_url: Some(
                "http://localhost:1455/auth/callback#code=fragment-code&state=fragment-state"
                    .to_string(),
            ),
            ..Default::default()
        })
        .expect("fragment callback url should parse");

        assert_eq!(payload.code.as_deref(), Some("fragment-code"));
        assert_eq!(payload.state.as_deref(), Some("fragment-state"));
    }

    #[test]
    fn accepts_raw_code_input() {
        let payload = parse_oauth_callback(&AuthExchangeInput {
            code: Some("4/0ArawGoogleCodeValue".to_string()),
            ..Default::default()
        })
        .expect("raw code should parse");

        assert_eq!(payload.code.as_deref(), Some("4/0ArawGoogleCodeValue"));
